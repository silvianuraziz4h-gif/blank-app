...
st.title("🧪 Kalkulator Pengenceran Larutan Pro")
st.write(
    "Kalkulator ini dapat menghitung salah satu variabel dari rumus pengenceran "
    "(**M₁ × V₁ = M₂ × V₂**) dengan konversi satuan otomatis."
)
...
with col1:
    ...
    if variabel_dicari != "Konsentrasi Awal / Stok (M₁)":
        m1 = st.number_input("Konsentrasi Awal (M₁)", min_value=0.0, step=0.1, format="%.4f", value=1.0)
    ...
    if variabel_dicari != "Volume Awal yang Dibutuhkan (V₁)":
        v1 = st.number_input("Volume Awal (V₁)", min_value=0.0, step=0.1, format="%.4f", value=10.0)
...
with col2:
    ...
    if variabel_dicari != "Konsentrasi Akhir / Target (M₂)":
        m2 = st.number_input("Konsentrasi Target (M₂)", min_value=0.0, step=0.1, format="%.4f", value=0.1)
...
try:
    if variabel_dicari == "Volume Awal yang Dibutuhkan (V₁)":
        if m1 <= 0:
            ...
            # Konversi semua ke satuan dasar
            ...
            # Rumus: V1 = (M2 * V2) / M1 (dalam mL dasar)
            v1_dasar = (m2_dasar * v2_dasar) / m1_dasar
            # Konversi balik ke satuan pilihan user
            v1_hasil = v1_dasar / KONVERSI_V[u_v1]
            
            # Hitung volume pelarut jika satuannya sama agar logis dicurahkan
            v2_konv_ke_v1_unit = v2_dasar / KONVERSI_V[u_v1]
            v_pelarut = v2_konv_ke_v1_unit - v1_hasil
            
            st.success(f"**Volume Awal (V₁) yang dibutuhkan:**")
            st.code(f"{v1_hasil:.4f} {u_v1}", language="text")
            
            if v_pelarut >= 0:
                st.info(f"💡 **Petunjuk:** Ambil **{v1_hasil:.4f} {u_v1}** Larutan Stok, lalu tambahkan pelarut sebanyak **{v_pelarut:.4f} {u_v1}** hingga volume total menjadi **{v2} {u_v2}**.")
        ...
        if v1 <= 0:
            ...
            st.code(f"{m1_hasil:.4f} {u_m1}", language="text")
        ...
        if m2 <= 0:
            ...
            st.code(f"{v2_hasil:.4f} {u_v2}", language="text")
        ...
        if v2 <= 0:
            ...
            st.code(f"{m2_hasil:.4f} {u_m2}", language="text")
